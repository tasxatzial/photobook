...
public class GetPosts extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < posts.size(); i++) {
            post = posts.get(i);
            json = new JSONObject();
            json.put("user_name", post.getUserName());
            json.put("description", post.getDescription());
            json.put("resource_url", post.getResourceURL());
            json.put("image_url", post.getImageURL());
            json.put("image_base64", post.getImageBase64());
            json.put("latitude", post.getLatitude());
            json.put("longitude", post.getLongitude());
            json.put("created_at", post.getCreatedAt());
